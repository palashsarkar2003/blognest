...
function CreateBlog() {
  ...
  const categories = [
    "Personal", "Business", "Devotional", "Nature", "Lifestyle",
    "Fashion", "Beauty", "Travel", "Food", "Health & Fitness",
    "Finance", "Parenting", "DIY & Crafts", "Technology", "Education",
    "Photography", "Music", "Sports", "Automotive", "Gaming",
    "Science", "Marketing", "News & Politics", "Affiliate & Review",
    "Art & Design", "Book & Writing", "SaaS (Software as a Service)",
    "Internet Services", "Religion & Spirituality", "Multi-author",
    "Niche/Hobby", "Pets", "Movie & Entertainment", "Environmental",
    "Gardening", "Mental Health", "Productivity", "Self-Improvement",
    "Parenting & Family", "Language Learning", "Cryptocurrency & Blockchain",
    "Real Estate", "Foodie & Gourmet", "Outdoor & Adventure",
    "Home Improvement", "Photography Tutorials", "Event Planning",
    "Career Advice", "Legal Advice", "College & University Life",
    "Celebrity & Pop Culture", "Education Technology", "Tech Reviews & Tutorials",
    "Mobile Apps", "Vegan & Vegetarian Lifestyle", "Wedding Planning",
    "Craft Beer & Alcohol", "Book Reviews", "Luxury Lifestyle",
    "Relationship Advice", "Fitness Challenges", "Science Fiction & Fantasy",
    "Anime & Comics", "Travel Photography", "Food Photography",
    "Sustainability", "Startup & Entrepreneurship",
  ];
  ...
  const handleImageChange = (e) => {
    ...
    const reader = new FileReader();
    reader.onloadend = () => {
      setBlogImage(reader.result); // Base64 string sent to backend
      setBlogImagePreview(reader.result); // Preview
    };
    reader.onerror = () => {
      toast.error("Error reading file!");
      setBlogImage("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const blogData = { title, category, about, blogImage };

      await axios.post(`${BACKEND_URL}/api/blogs/create`, blogData, {
        withCredentials: true,
        headers: {
